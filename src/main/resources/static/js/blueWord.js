  ...
  var WordFindGame = function() {
    ...
    */
    var endTurn = function () {
    	var lastSquare = selectedSquares[selectedSquares.length-1];
    	var y1 = $(startSquare).attr('x')-0;
        var x1 = $(startSquare).attr('y')-0;
        var y2 = $(lastSquare).attr('x')-0;
        var x2 = $(lastSquare).attr('y')-0;
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
        	for (const key in map) {
        		  if (key == curWord || reverseString(curWord) == key) {
        			  if (key == curWord) {
        				  var x = map[key];
        				  if (x1 != x[0] || y1 != x[1] || x2 != x[2] || y2 != x[3]) {
        	        			skip = true;
        	        	  }
        			  } else {
        				  var x = map[key];
        				  ...
        				  console.log(x);
        				  if (x1 != x[2] || y1 != x[3] || x2 != x[0] || y2 != x[1]) {
        	        			skip = true;
        	        	  }
        			  }
        		  }
    ...
    */
    ...
    return {
      ...
      */
      create: function(inputted, words, puzzleEl, wordsEl, options) {
...
$(function () {
  ...
  var gamePuzzle = wordfindgame.create(puzzle,
    words, 
    '#puzzle', 
    '#words', 
    { height: 8, 
